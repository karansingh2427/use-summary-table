...
// a content-safety tripwire — shared by functions/api/extract.js and
// functions/api/jobs.js. Ported/duplicated at worker/src/governance.js for
// the standalone Worker (no shared build step between the two projects —
// same reason worker/src/pipeline.js is a documented ported copy of
// app/index.html's extraction logic; keep both files in sync by hand).
//
...
// cost-control gaps. Requires the AUDIT_LOG binding from wrangler.toml.
//
...
// request body, so a call that escalated to Opus (see worker/src/pipeline.js
// / app/index.html's remediation loop) prices at Opus rates, not a blended
// Sonnet-only guess.
const MODEL_PRICING = {
 ...
 * extraction on both paths in seconds — no code change, no redeploy, no
 * waiting on a Pages "Retry deployment" to pick up a new env var. Toggle it
 ...
 * so /api/extract and /api/jobs don't share one counter. Fails open (allows
 * the request) if the KV check itself errors — a governance-layer outage
 * must never take down extraction.
 */
